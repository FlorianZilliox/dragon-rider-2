...
**Avant la carte, la salle d'essai.** Une idée de jeu (un bûcher, une énigme) se prouve jouable dans une salle d'essai (`niveaux/salles/<nom>.txt`, `#salle=<nom>`), au robot puis entre les mains de l'utilisateur, avant d'entrer dans un niveau. Jamais de fiche papier à lui faire lire (voir le skill `jeu-scenariste`).
...
- **La hauteur se gagne dans des puits larges ou par des courants ascendants**, avec des rebords réguliers. Mieux qu'une colonne toute faite : un **bûcher** au fond du puits, que le joueur allume lui-même ; il éclaire la salle sombre et devient son chemin pour remonter (Dragon Rider, Tours). Aucune montée de plus de 40 % du souffle sans un endroit où se poser.
- **Les îles** (fragments de tour qui flottent) portent dessous un coin de roche dont la collision (cases `u`) est calculée d'après ses pixels : ce qu'on voit est ce qui arrête. Les flèches des tours sont un décor, sans pics.
