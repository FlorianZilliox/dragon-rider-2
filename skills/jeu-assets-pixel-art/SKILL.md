...
|---|---|---|
| Brief | `art/briefs/<nom>.json` (vérifier que le nom est libre : réécrire un brief existant détourne son image d'origine) | Sujet, cadrage, fond (transparent pour un objet), références (une liste). Formuler sobrement : la modération rejette les briefs violents. Pour du texte (logo, inscription), épeler les lettres et vérifier l'orthographe sur l'image (une génération a déjà sorti « DRAGOON », une autre un accent en trop). |
...
- **Le sol sur lequel se dressent les structures se peint d'après elles** : l'image des tours sert de référence de forme au chemin de ronde, qui prend leur pierre, leur valeur et leurs créneaux. Un parapet gris clair sous des tours noires faisait « collage ».
- **Les pieds se posent dans le sol, pas sur son bord** : la ligne des pieds se choisit dans l'image du sol (au milieu des dalles d'un chemin de ronde), et les objets posés (tour, bûcher) partagent cette ligne.
- **Une structure posée interrompt le parapet** sous sa silhouette (le dessus du chemin passe dessous) : elle ne mord pas sur les créneaux.
